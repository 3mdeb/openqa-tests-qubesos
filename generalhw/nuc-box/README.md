...
```
openqa-cli api -X POST isos DISTRI=qubesos VERSION=4.2.3 ARCH=x86_64 BUILD=4.2.3 FLAVOR=install-iso-nuc-box WORKER_CLASS=nuc-box CASEDIR="https://github.com/3mdeb/openqa-tests-qubesos.git#nuc-box" NEEDLES_DIR=%%CASEDIR%%
```

ISO name is generated as `ISO=Qubes-R%BUILD%-%ARCH%.iso`, and a file with this
name needs to exist on the openQA worker, but in practice that file goes unused
- that file could well be empty, and the ISO image PiKVM presents to the SUT
...
- the installation phase gets stuck while trying to perform package
  configuration, always with the "alsa-sof-firmware" package.

Note: this platform comes with an NVMe drive. [The following
